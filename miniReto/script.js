...
const totalPrs = document.getElementById('cantidadP');
const totalPagar = document.getElementById('totalP');
...
form.addEventListener('submit', function(event) {
  ...
  const personas = parseInt(totalPrs.value);
  
  if (isNaN(bill) || isNaN(tipPercentage) || isNaN(personas) || bill <= 0) {
  ...
  //Calcular pagos personas
  const calculoP = totalAmount/personas;
  ...
  totalPagar.textContent = calculoP.toFixed(2)
